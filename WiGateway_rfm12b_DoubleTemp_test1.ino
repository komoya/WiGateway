...
typedef struct {         //struct from node1
  int temp; 
  int battery;
} 
PayloadTx;
...
typedef struct {         //struct from node2
  int temp; 
  int battery;
} 
PayloadTx2;
...
void loop() {
  ...
  if (rf12_recvDone())  //接收数据开始
  {
    if (rf12_crc == 0 && (rf12_hdr & RF12_HDR_CTL) == 0)  // and no rf errors/检查RF没有错误，如没有错误，继续
    {
      ...
      if (node_id == 18) {            //for the node1
        emontx = *(PayloadTx*) rf12_data;
        Serial.print("received ok");
      }
      if (node_id == 19) {             //for the node2
        emontx2 = *(PayloadTx2*) rf12_data;
        Serial.print("received ok");
      }
  ...
  tempW = emontx.temp/100; //temp from node1
  tempF = emontx.temp%100;
  battery = emontx.battery; 

  tempW2 = emontx2.temp/100; //temp from node2
  tempF2 = emontx2.temp%100;
  battery2 = emontx2.battery; 

    // 得到tempnode的温度。需要除以100
  ...
  if(pos) {
    ...
    bfill.emit_p(PSTR("HTTP/1.0 200 OK\r\n"
     ...
     "Temp Inside: " //temp 2
      ...
      "</body></html>"),tempW,tempF,battery,tempW2,tempF2,battery2);
